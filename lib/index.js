...
export function apply(ctx) {
  ...
  async function loadAssets() {
    ...
    const candidates = [
      ...
      '/root/workspace/pet_assets.json',
    ...
    for (const p of candidates) {
      if (fs.existsSync(p)) {
        try {
          ...
          console.error('[dsh-desktop-pet] assets read error', e);
        }
      }
    }
    return {};
  ...
  async function listModels() {
    const result = [];
    const candidates = [
      path.resolve(process.cwd(), 'configured_models.json'),
      '/root/workspace/configured_models.json'
    ];
    for (const p of candidates) {
      if (fs.existsSync(p)) {
        try {
    ...
    if (llm) {
      try {
        ...
        for (const p of providers) {
          try {
            ...
            for (const m of models) {
              if (!result.some(r => r.provider === p.id && r.model === m.id)) {
                result.push({
                  provider: p.id,
                  model: m.id,
                  name: m.name || m.id,
        ...
        console.error('[dsh-desktop-pet] list-models llm error', e);
      }
    }
  ...
  // 3. Durable File Storage (Load)
  async function loadDurableData() {
    const p = path.resolve(process.cwd(), 'pet_durable_memory.json');
    if (fs.existsSync(p)) {
      try {
        const raw = fs.readFileSync(p, 'utf8');
  ...
  async function saveDurableData(data) {
    ...
    try {
      ...
      console.error('[dsh-desktop-pet] save-durable-data error', e);
    ...
    return new Promise((resolve) => {
      speakQueue = speakQueue.then(async () => {
        try {
          ...
          console.error('[dsh-desktop-pet] speak error', err);
          resolve({
            mood: 'hungry',
  ...
  if (connection && connection.fetch && typeof connection.fetch.register === 'function') {
    ...
    }), 'dsh-desktop-pet: durable data route');
