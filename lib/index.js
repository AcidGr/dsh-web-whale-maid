...
export function apply(ctx) {
  ...
  async function listModels() {
    ...
    // C. If defaultModel is matched, place it at the front of the list
    if (defaultModel) {
    ...
    return {
      models: result,
      defaultModel: defaultModel || result[0] || null
    };
  }
    ...
    return new Promise((resolve) => {
      speakQueue = speakQueue.then(async () => {
        try {
          ...
          // Fallback to active DSH default model if not specified
          if (!provider || !model) {
            const defAgent = ctx.get('agentDefaultModel')?.currentSelection?.();
            if (defAgent && defAgent.provider && defAgent.model) {
              provider = defAgent.provider;
              model = defAgent.model;
            }
